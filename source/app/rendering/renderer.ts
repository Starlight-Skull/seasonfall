...
export function drawMain (ctx: CanvasRenderingContext2D): void {
  ...
  saveRestore(ctx, () => {
    ...
    if (world.showLiveDebug) drawTextWithBackground(ctx, `${render.mouseX},${render.mouseY}`, grid(render.mouseX), grid(render.mouseY), { color: 'white' })
    //* focus point *//
 ...
 */
function saveRestore (ctx: CanvasRenderingContext2D, script: () => void): void {
  ctx.save()
  script()
  ctx.restore()
}
 ...
 */
function drawStats (ctx: CanvasRenderingContext2D, entity: Entity): void {
  let x = grid(entity.x + entity.width / 2)
  let y = grid(entity.y)
  if (entity instanceof Hero) {
    //* name *//
    drawTextWithBackground(ctx, entity.heroName, x, y - 65, { color: 'rgb(255,255,255)', center: true })
    //* xp *//
    if (entity.stats.xp !== 0) {
      drawTextWithBackground(ctx, `${entity.stats.xp}`, x, y - 95, { color: 'rgb(0,255,0)', center: true })
    }
  } else {
    //* hp *//
    ...
    if (world.showLiveDebug) {
      ...
      drawTextWithBackground(ctx, val, x, y - 95, { color: 'rgb(255,255,255)', center: true })
    }
  }
}
...
function drawDebug (ctx: CanvasRenderingContext2D): void {
  ...
  if (world.showLiveDebug) {
    ...
    drawTextWithBackground(ctx, `ANIM: ${tracked.name}::${tracked.animation.name} - ${Math.round(tracked.animationFrame * 100) / 100 + 1}/${tracked.animation.frames}`, 5, 100, { color: 'cyan' })
    drawTextWithBackground(ctx, `POS: [${Math.round(tracked.x)}, ${Math.round(tracked.y)}] \t ${tracked.collision.enabled ? 'COL: ' : ''}[${tracked.collision.left ? ' ←' : ''}${tracked.collision.up ? ' ↑' : ''}${tracked.collision.down ? ' ↓' : ''}${tracked.collision.right ? ' →' : ''}]`, 5, 130, { color: 'cyan' })
    drawTextWithBackground(ctx, `SHADE: ${Math.round(world.shade * 100) / 100} \t MOVE: [${tracked.movement.left ? ' ←' : ''}${tracked.movement.attack ? ' $' : ''}${tracked.movement.use ? ' #' : ''}${tracked.movement.jump ? ' ▲' : ''}${tracked.movement.down ? ' ↓' : ''}${tracked.movement.right ? ' →' : ''}] ${tracked.stats.jumpTime}`, 5, 160, { color: 'cyan' })
    drawTextWithBackground(ctx, `WORLD: [${level.properties.rootX},${level.properties.rootY}][${level.properties.borderX},${level.properties.borderY},${level.properties.borderW},${level.properties.borderH}]`, 5, 190, { color: 'cyan' })
    drawTextWithBackground(ctx, `DEBUG: ${world.debug}`, 5, 220, { color: 'red' })
  }
  if (world.showPlayerStats) {
    drawTextWithBackground(ctx, `Attacks: ${playerStats.attacks}`, 5, 220, { color: 'magenta' })
    drawTextWithBackground(ctx, `Attacks Hit: ${playerStats.attacksHit}`, 5, 250, { color: 'magenta' })
    drawTextWithBackground(ctx, `Damage Taken: ${playerStats.damageTaken}`, 5, 280, { color: 'magenta' })
    drawTextWithBackground(ctx, `Damage Dealt: ${playerStats.damageDealt}`, 5, 310, { color: 'magenta' })
    drawTextWithBackground(ctx, `Kills: ${playerStats.kills}`, 5, 340, { color: 'magenta' })
    drawTextWithBackground(ctx, `Time Taken: ${playerStats.timeTaken}`, 5, 370, { color: 'magenta' })
  }
}
 ...
 */
function drawWorld (ctx: CanvasRenderingContext2D): void {
  //* tiles *//
